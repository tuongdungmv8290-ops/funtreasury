...
export function GiftDialog({ open, onOpenChange, defaultReceiverId, postId }: GiftDialogProps) {
  ...
  const renderBalanceCard = () => (
    <div className="p-3 rounded-xl bg-gradient-to-r from-yellow-100 to-amber-50 dark:from-yellow-900/30 dark:to-amber-900/20 border border-yellow-300/50 dark:border-yellow-700/40">
      ...
      {!isInternal && wallet.isConnected && wallet.address && (
        <p className="text-[10px] text-yellow-600/70 dark:text-yellow-500/70 font-mono mt-0.5">
        ...
        <DialogContent className="max-w-md p-0 overflow-hidden">
          ...
          <div className="px-6 pb-6 space-y-4">
            ...
            <Tabs value={activeTab} onValueChange={(v) => { setActiveTab(v); setDropdownOpen(false); }}>
              <TabsList className="w-full grid grid-cols-2 h-11 rounded-xl bg-secondary/50">
                <TabsTrigger value="internal" className="rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-md text-sm font-medium">
                  🎁 Camly Coin
                </TabsTrigger>
                <TabsTrigger value="crypto" className="rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-md text-sm font-medium">
                  ⛓️ Chuyển Crypto
                </TabsTrigger>
              </TabsList>
              ...
              <TabsContent value="internal" className="space-y-4 mt-4">
                ...
                {/* Amount */}
                <div className="space-y-2">
                  <Label>Số lượng (tối thiểu 100)</Label>
                ...
                {/* Message */}
                <div className="space-y-2">
                  <Label>Lời nhắn yêu thương (tuỳ chọn)</Label>
                  <div className="relative">
                ...
                <Button onClick={handleSend} disabled={!canSubmit}
                  className="w-full h-12 text-base font-bold gap-2 bg-gradient-to-r from-amber-500 via-yellow-400 to-amber-500 text-white shadow-lg hover:shadow-xl transition-all">
              ...
              <TabsContent value="crypto" className="space-y-4 mt-4">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2 text-xs">
                      ...
                      <span className={cryptoReceiverMode === 'address' ? 'font-semibold text-foreground' : 'text-muted-foreground'}>Địa chỉ ví</span>
                      <Switch
                        checked={cryptoReceiverMode === 'profile'}
                        onCheckedChange={checked => setCryptoReceiverMode(checked ? 'profile' : 'address')}
                        className="scale-75"
                      />
                      <Users className="w-3.5 h-3.5 text-muted-foreground" />
                      <span className={cryptoReceiverMode === 'profile' ? 'font-semibold text-foreground' : 'text-muted-foreground'}>Từ hồ sơ</span>
                    </div>
                  </div>

                  {cryptoReceiverMode === 'address' ? (
                    <Input placeholder="0x..." value={manualAddress}
                      onChange={e => setManualAddress(e.target.value)} className="font-mono text-sm" />
                  ) : (
                    <>
                      {renderProfileSearch()}
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-orange-500/80 flex items-center gap-1">
                    <AlertTriangle className="w-3 h-3" /> Cần có BNB để thanh toán phí gas
                  </p>
                </div>
                ...
                {/* Submit */}
                <Button onClick={handleSend} disabled={!canSubmit}
                  className="w-full h-12 text-base font-bold gap-2 bg-gradient-to-r from-orange-500 via-amber-500 to-orange-500 text-white shadow-lg hover:shadow-xl transition-all">
                  {isSending ? <Loader2 className="w-5 h-5 animate-spin" /> : (
                    <>Xác nhận chuyển <ArrowRight className="w-5 h-5" /></>
                  )}
                </Button>
              </TabsContent>
            </Tabs>
